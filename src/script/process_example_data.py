...
import numpy as np
from plot_multiple import basisvec_one_plot
...
def standard_process(data_vec, baseline, max_vec = None):
    ...
    subtractbase_data_vec = data_vec - baseline
    target_base_data = np.mean(np.absolute(subtractbase_data_vec))
    if max_vec != None:
...
def process_state4_5(path,SET_TRAILS,count,baseline,max_vec):
    ...
    for trail_index in range (0,count):
        ...
        if (indexof_lpl4 != 0 and indexof_lpl4.size != 0):
